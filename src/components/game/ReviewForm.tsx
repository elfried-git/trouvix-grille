      ...
      <DialogContent className="glass-card max-w-md rounded-2xl border-amber-400/30 p-6">
        {success ? (
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            ...
            <div>
              <p className="font-display text-xl font-bold text-amber-100">Merci pour ton avis ! ⭐</p>
              <p className="mt-1 text-sm text-muted-foreground">Ton avis sera visible après sa validation.</p>
            </div>
            <Button onClick={handleClose} className="mt-2 bg-gradient-to-r from-amber-500 to-amber-400 text-black hover:from-amber-400 hover:to-amber-300">Fermer</Button>
          </div>
        ) : (
          <>
            <DialogHeader>
              <DialogTitle className="font-display text-xl font-bold text-gold-gradient">Laisser un avis</DialogTitle>
              <DialogDescription>Partage ton expérience avec Trouvix Grille. Tous les avis sont modérés par Benchou Ferrari.</DialogDescription>
            </DialogHeader>
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="review-name" className="text-sm text-amber-100/90">Ton nom</Label>
                <Input id="review-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Votre nom" maxLength={40} className="bg-background/60" autoComplete="off" />
              </div>
              <div className="flex flex-col gap-1.5">
            ...
            <DialogFooter className="gap-2">
              <Button variant="outline" onClick={handleClose} disabled={loading} className="border-amber-400/30 bg-amber-500/5 text-amber-100 hover:bg-amber-500/15">Annuler</Button>
              <Button onClick={handleSubmit} disabled={!isValid || loading} className="bg-gradient-to-r from-rose-600 to-rose-500 text-white hover:from-rose-500 hover:to-rose-400">
                {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Envoi…</> : <><Sparkles className="mr-2 h-4 w-4" /> Publier mon avis</>}
              </Button>
            </DialogFooter>
          </>
        )}
