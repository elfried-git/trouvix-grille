...
const RATE_LIMIT_MAX = 3; // 3 reviews per window per IP
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (trimmedComment.length < 3 || trimmedComment.length > 500) {
      return NextResponse.json(
        { error: "Le commentaire doit contenir entre 3 et 500 caractères" },
        { status: 400 }
      );
    }
    ...
    if (containsProfanity(trimmedName) || containsProfanity(trimmedComment)) {
      return NextResponse.json(
        { error: "Votre avis contient des mots inappropriés. Merci de le reformuler." },
        { status: 400 }
      );
    }
    ...
    // Duplicate detection (same name + same comment in the last hour)
    // SQLite doesn't support `mode: "insensitive"` so we query the last hour's
    // pending reviews and filter case-insensitively in JS.
    ...
    const existing = recent.find(
      (r) =>
        ...
        r.comment.toLowerCase() === lowerComment
    );
    if (existing) {
      return NextResponse.json(
        { error: "Vous avez déjà envoyé cet avis." },
        { status: 400 }
      );
    }

    // Create the review (status pending by default)
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Build distribution {5: n, 4: n, 3: n, 2: n, 1: n}
